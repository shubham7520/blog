...
app.get("/", function (req, res) {
  Post.find({}, function (err, posts) {
    res.render("home", { startingContant: homeStartingContent, posts: posts });
  });
});
...
app.get("/posts/:postId", function (req, res) {
  ...
  Post.findOne({ _id: requestPostId }, function (err, post) {
    if (!err) {
      res.render("post", { title: post.title, contant: post.contant });
    } else {
      res.render("post", {
        title: "404",
        contant: "No such post exist in our server!",
      });
    }
  });
});
